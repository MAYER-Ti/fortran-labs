...
program ex_1_1
   ...
   implicit none
   integer, parameter         :: BLOCK_LEN = 15, EMPLOYEE_COUNT = 6
   character(:), allocatable  :: input_file, output_file, format
   
   ! Массивы фамилий и должностей 
   character(kind=CH_) :: surnamesEmployee(BLOCK_LEN,EMPLOYEE_COUNT) = "", &
                          positionsEmployee(BLOCK_LEN,EMPLOYEE_COUNT) = ""
   
   logical, allocatable                        :: Mask(:)

   integer :: IO, i, j
!   integer, parameter                          :: INDEXES(*) = [(i, i = 1, EM)]

   input_file = "../data/class.txt"
   output_file = "output.txt"
   ! Чтение списка класса: фамилии, должности.
   call Read_employee_list(input_file, BLOCK_LEN, EMPLOYEE_COUNT, &
      surnamesEmployee, positionsEmployee)
   ! Вывод списка класса.
   call Write_employee_list(output_file, BLOCK_LEN, EMPLOYEE_COUNT, &
      surnamesEmployee, positionsEmployee, "input data:", "append") 
!Write_employee_list(output_file, BLOCK_LEN, EMPLOYEE_COUNT, surnamesEmployee, positionEmployee, listName, positionToWrite)
...
contains
   ...
   subroutine Write_employee_list(output_file, BLOCK_LEN, EMPLOYEE_COUNT, &
      ...
      open (file=output_file, encoding=E_, position=positionToWrite, newunit=Out)
         write (Out, '(/a)') listName 
         format = '('// BLOCK_LEN // 'a1, 1x, ' // BLOCK_LEN // 'a1)'
         write (Out, format, iostat=IO) (surnamesEmployee(:,i), positionsEmployee(:,i), &
            i = 1, EMPLOYEE_COUNT)
